...
parser.add_argument('--node_hidden_size', type=int, default=32)
...
parser.add_argument('--num_step_message_passing', type=int, default=2)  # DMP
...
parser.add_argument('--loss1_weight', type=float, default=2)  # lambda
...
def get_node_attr_bhgeo(filepath):
    ...
    for line in fr1.readlines():
        ...
        temp_node_hop = str_list[2]#hopxxxx
...
def build_graph_bhgeo(filepath):
    ...
    for line in fr1.readlines():
        ...
        temp_node_attr = list(map(eval, str_list[2:]))
        temp_node_delay = str_list[2]
        temp_node_hop = str_list[2]#hop xxxx
...
def train_test_val_targetscaler_bh(filepath):#
    ...
    return [train_labeled_nodes, new_lat_train_labels, new_lon_train_labels, lat_train_labels, lon_train_labels, delay_train_labeled_nodes,hop_train_labeled_nodes, delay_train_labels,hop_train_labels,
            val_labeled_nodes, new_lat_val_labels,new_lon_val_labels,lat_val_labels, lon_val_labels, delay_val_labels,hop_val_labels,
            test_labeled_nodes, new_lat_test_labels,new_lon_test_labels,lat_test_labels, lon_test_labels , delay_test_labels,hop_test_labels]
...
if __name__ == '__main__':
    ...
    stamp = int(time())
    result_path1 = filepath + '/output_0805/' + str(stamp) + '_loss.txt'
    if not os.path.exists("./output_0805/"):
        os.makedirs("./output_0805/")
    ...
    G, edge_attr_array = build_graph_bhgeo(filepath + '\\data_0120\\sh_edge_feature_20220120_0906.txt')
    node_attr_array = get_node_attr_bhgeo(filepath + '\\data_0120\\sh_ip_feature_20220120_0906.txt')
    ...
    train_labeled_nodes, new_lat_train_labels, new_lon_train_labels, lat_train_labels, lon_train_labels, delay_train_labeled_nodes, hop_train_labeled_nodes, delay_train_labels,hop_train_labels,\
    val_labeled_nodes, new_lat_val_labels, new_lon_val_labels, lat_val_labels, lon_val_labels, delay_val_labels,hop_val_labels,\
    test_labeled_nodes, new_lat_test_labels, new_lon_test_labels, lat_test_labels, lon_test_labels, delay_test_labels,hop_test_labels=\
        train_test_val_targetscaler_bh(filepath + '\\data_0120\\' + "sh_dstip_id_allinfo_20220119_1510.txt")
    ...
    if use_cuda:
        ...
        train_labeled_nodes, new_lat_train_labels, new_lon_train_labels, lat_train_labels, lon_train_labels, delay_train_labeled_nodes, hop_train_labeled_nodes, delay_train_labels, hop_train_labels, \
            val_labeled_nodes, new_lat_val_labels, new_lon_val_labels, lat_val_labels, lon_val_labels, delay_val_labels, hop_val_labels, \
            test_labeled_nodes, new_lat_test_labels, new_lon_test_labels, lat_test_labels, lon_test_labels, delay_test_labels, hop_test_labels = \
            train_labeled_nodes.cuda(), new_lat_train_labels.cuda(), new_lon_train_labels.cuda(), lat_train_labels.cuda(), lon_train_labels.cuda(), delay_train_labeled_nodes.cuda(), hop_train_labeled_nodes.cuda(),delay_train_labels.cuda(), hop_train_labels.cuda(),\
            val_labeled_nodes.cuda(), new_lat_val_labels.cuda(), new_lon_val_labels.cuda(), lat_val_labels.cuda(), lon_val_labels.cuda(), delay_val_labels.cuda(), hop_val_labels.cuda(),\
            test_labeled_nodes.cuda(), new_lat_test_labels.cuda(), new_lon_test_labels.cuda(), lat_test_labels.cuda(), lon_test_labels.cuda(), delay_test_labels.cuda(), hop_test_labels.cuda()
    ...
    for epoch in range(epochs):
        ...
        y4_train_loss = loss_fn(y3_predict[hop_train_labeled_nodes],
                                hop_train_labels.squeeze(-1))  # delay_train_labeled_nodes除了train还包含了中间路由器的id
        ...
        with torch.no_grad():
            ...
            if 1:#:(epoch%10==1):  #
                ...
                for i, temp in enumerate(lat_val_labels):
                    ...
                    temp_hop_label= hop_val_labels[i].item()
                    temp_delay_pre = temp_y3_predict[i]
                ...
                y4_predict_copy = y4_predict.clone()

                y4_predict_copy[hop_train_labeled_nodes] = hop_train_labels.squeeze(-1)

                y4_predict_copy[val_labeled_nodes] = hop_val_labels.squeeze(-1)
